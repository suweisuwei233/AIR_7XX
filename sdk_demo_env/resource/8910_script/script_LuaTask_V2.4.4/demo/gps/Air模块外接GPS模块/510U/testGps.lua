--- 模块功能：GPS功能测试(本demo是以Air720通过uart1外接Air510U为例配置，注意：Air510供电要求3.3V，Air720的IO是1.8V，硬件上注意做电平转换).
-- @author openLuat
-- @module gps.testGps
-- @license MIT
-- @copyright openLuat
...
--agps功能模块只能配合Air800、Air510或者Air530使用；如果不是这三款模块，不要打开agps功能
local agps=require"agpsHxxt"
s = "$AIDINFO"
...
--如下两部分是以Air720通过uart1外接Air510U为例的配置代码
--gps.setPowerCbFnc，设置供电开关回调，实际测试时，使用的是直流电源直接给Air510U供电，所有回调中没有实现任何有效代码
-- gps.setPowerCbFnc(
--     function(status)
--         if status then
--             --打开供电
...
--     end
-- )
--gps.setPowerCbFnc，设置串口通信参数，Air510U的波特率为9600
