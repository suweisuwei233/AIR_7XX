            ...
            for kk, vv in pairs(v) do
                if type(vv) == "table" then
                    for i = 1, #vv do
                        print(kk, vv[i])
                        table.insert(body,
                            "--" .. boundary .. "\r\nContent-Disposition: form-data; name=\"" .. kk .. "\"; filename=\"" ..
                                vv[i]:match("[^%/]+%w$") .. "\"\r\nContent-Type: " ..
                                contentType[vv[i]:match("%.(%w+)$")] .. "\r\n\r\n")
...
sys.taskInit(function()
    ...
    while 1 do
        ...
        --                                 req_headers, body -- POST请求所需要的body, string, zbuff, file均可
        -- ).wait()
        -- log.info("http.post", code, headers, body)

        -- 也可用postMultipartFormData(url, params) 上传文件
        -- postMultipartFormData("http://airtest.openluat.com:2900/uploadFileToStatic", {
        --     -- texts =
        --     -- {
